        ...
        <div className="print-invoice-sheet p-8 sm:p-10 space-y-5 bg-white text-slate-950 font-sans print:p-0 print:space-y-3.5">
          ...
          <div className="flex justify-between items-start gap-4">
            ...
            <div className="text-right shrink-0">
              ...
              <p className="text-[10px] uppercase font-bold tracking-widest text-slate-500 font-mono mt-0.5">
                {isGst ? (gstType === 'inter' ? 'Inter-State Supply' : 'Intra-State Supply') : 'Exempt / Non-GST Supply'}
              </p>
            </div>
          </div>

          {/* Metadata Section: To (Client) & Invoice Metadata (No, Date, Due Date, POS, RCM) */}
          <div className="flex justify-between items-start gap-4 pt-1">
            <div className="space-y-0.5 max-w-sm">
              <span className="text-xs font-semibold text-slate-700">Billed To:</span>
              ...
              {clientGstin && (
                <p className="text-xs text-slate-900 font-semibold font-mono pt-0.5">
                  <span className="font-bold font-sans">GSTIN:</span> {clientGstin}
                </p>
              )}
              {clientPan && (
                <p className="text-xs text-slate-900 font-semibold font-mono">
                  <span className="font-bold font-sans">PAN:</span> {clientPan}
                </p>
              )}
              {placeOfSupply && (
            ...
            <div className="text-xs space-y-1 text-right shrink-0">
              <div className="flex justify-end gap-3">
                <span className="font-semibold text-slate-700 min-w-[95px] text-left">Invoice No.:</span>
                <span className="font-bold text-slate-950 font-mono text-right min-w-[85px]">{invoice.invoice_no || 'INV-0001'}</span>
              </div>
              <div className="flex justify-end gap-3">
                <span className="font-semibold text-slate-700 min-w-[95px] text-left">Invoice Date:</span>
                <span className="font-medium text-slate-900 text-right min-w-[85px]">{formatInvoiceDate(invoice.issue_date)}</span>
              </div>
              <div className="flex justify-end gap-3">
                <span className="font-semibold text-slate-700 min-w-[95px] text-left">Due Date:</span>
              ...
              <div className="flex justify-end gap-3 pt-0.5 border-t border-slate-200">
                <span className="font-semibold text-slate-700 min-w-[95px] text-left">Reverse Charge:</span>
                <span className="font-bold text-slate-900 text-right min-w-[85px]">{isRcm}</span>
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-y border-slate-300 font-bold text-[11px] uppercase tracking-wider text-slate-900">
                  ...
                  <th className="py-2 px-2 text-center w-24">SAC / HSN</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {invoice.items && Array.isArray(invoice.items) && invoice.items.length > 0 ? (
                    ...
                    return (
                      <tr key={idx}>
                        <td className="py-2 px-2 font-medium text-slate-900">
                          {it.particulars || 'Tax Advisory Services'}
                        </td>
                  ...
                  <tr>
                    <td colSpan={5} className="py-3 px-2 text-center text-slate-400 italic">No line items listed.</td>
                  </tr>
                )}
              </tbody>
            </table>
            ...
            <div className="space-y-1 text-xs">
              ...
              {isGst && (
                <>
                  {gstType === 'inter' ? (
                    <div className="flex justify-between text-slate-700">
                      <span>IGST ({parsedGstRate}%):</span>
                      <span className="font-mono font-medium text-slate-950">{formatCurrency(igst)}</span>
                    </div>
                  ) : (
                    <>
                      <div className="flex justify-between text-slate-700">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-3 border-t border-slate-200">
            ...
            {/* Terms & Conditions */}
            <div className={`space-y-1 text-[11px] text-slate-800 ${(organization?.bank_name || organization?.bank_account_number) ? '' : 'sm:col-span-2'}`}>
            ...
            {(organization?.bank_name || organization?.bank_account_number) && (
              <div className="text-[11px] text-slate-800 space-y-1 sm:text-right">
                <h4 className="font-bold uppercase text-[10px] text-slate-950 tracking-wider mb-0.5">
                  BANK DETAILS:
                </h4>
